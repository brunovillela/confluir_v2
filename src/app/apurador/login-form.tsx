...
import { confirmarCodigoApurador, enviarCodigoApurador } from "./actions"

export function LoginApurador() {
  const [email, setEmail] = useState("")
  const [enviado, setEnviado] = useState(false)
  const [estEnviar, actEnviar, pendEnviar] = useActionState(
    async (prev: { erro?: string; ok?: string }, fd: FormData) => {
      const r = await enviarCodigoApurador(prev, fd)
